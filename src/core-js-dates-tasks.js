...
function getNextFriday(date) {
  ...
  const daysUntilNextFriday = (5 - current.getDay() + 7) % 7;
  current.setTime(
    current.getTime() +
      (daysUntilNextFriday === 0 ? 7 : daysUntilNextFriday) *
        24 *
        60 *
        60 *
        1000
...
function getQuarter(date) {
  ...
  return +Object.keys(quarter).find((key) => quarter[key].includes(month));
}
